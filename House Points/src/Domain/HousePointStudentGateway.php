...
class HousePointStudentGateway extends QueryableGateway
{
    ...
    public function selectStudentPoints($studentID, $yearID) {
        $select = $this
            ->newSelect()
            ->from('hpPointStudent')
            ->cols(['hpPointStudent.hpID','DATE_FORMAT(hpPointStudent.awardedDate, \'%d/%m/%Y\') AS awardedDate','hpPointStudent.points','hpCategory.categoryName', 'hpCategory.categoryEvent', 'hpPointStudent.reason','CONCAT(gibbonPerson.title, \' \', gibbonPerson.preferredName, \' \', gibbonPerson.surname) AS teacherName'])
